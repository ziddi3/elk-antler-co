...
export default async function handler(req, res) {
  ...
  for (const requested of requestedItems) {
    ...
    if (!product) {
      return res.status(400).json({ error: `Unknown product: ${requested.id || 'missing id'}` })
    }
    ...
    if (!variant) {
      return res.status(400).json({ error: `Unknown variant for product ${product.id}` })
    }
  ...
  const subtotal = Number(authoritativeItems
  ...
  const shipping = subtotal > 0 && freeShippingThreshold > 0 && subtotal >= freeShippingThreshold
    ? 0
    : shippingFlatRate
  const tax = Number((subtotal * gstRate).toFixed(2))
  const total = Number((subtotal + shipping + tax).toFixed(2))
  const origin = req.headers.origin || 'https://birchamelkantler.com'
